...
import { currentUser, sparkRTC } from '../../pages/meeting.js'
...
const stageWidth = computed(() => windowWidth.value - (attendeesWidth + 20) - 130)
...
export const Stage = () => {
  ...
  return html`<div class="transition-all relative h-full px-4 lg:px-10" style="width: calc(100% - ${attendeesWidth + 20}px);">
    <div class="flex gap-4 flex-wrap justify-center items-center h-full">
      ${Object.values(streamers.value).map((attendee, i) => {
        return html`<div
          key=${i}
          style="width: ${itemsWidth.value}px"
          class=${clsx('transition-all aspect-video relative max-w-full min-w-[100%] sm:min-w-[unset] text-white-f-9', 'bg-gray-1 rounded-lg min-w-10', 'dark:bg-gray-3 dark:')}
        >
          <${Video} stream=${attendee.stream} userId=${attendee.userId} isMuted=${currentUser.value.isMeetingMuted} name=${attendee.name} isHostStream=${attendee.isHost} />
        </div>`
      })}
    </div>
  </div>`
...
export const Video = memo(({ stream, isMuted, isHostStream, name, userId }) => {
  ...
  return html`<video ref=${videoRef} autoplay muted="${muted}" className="w-full h-full object-cover rounded-lg" />
    <div class="px-4 py-1 bg-black bg-opacity-50 text-white rounded-full absolute top-3 left-3 text-medium-12">${name} ${isHostStream && '(Host)'}</div>
    ${isHost &&
    !isHostStream &&
    html` <${IconButton} class="absolute top-3 right-3" variant="ghost" onClick=${handleOpenMenu} ref=${menu}>
      ...
      ${
        ...
        html`<div class="relative top-0 right-0 h-full w-full">
          <ul class="bg-white absolute top-0 right-0 mt-3 -ml-2 text-black rounded-sm p-1">
            <li class="w-full whitespace-nowrap px-4 py-1 rounded-sm bg-black bg-opacity-0 hover:bg-opacity-10" onClick=${handleRemoveStream}>Stop broadcast</li>
          </ul>
        </div>`
      }
      <div>
