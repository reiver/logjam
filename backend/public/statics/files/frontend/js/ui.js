...
function mapping(positionsAndSizes, participants) {
    ...
    for (const div of Array.from(videoContainers)) {
        ...
        videosMap.set(id === 'localVideo' ? div.querySelector('video').srcObject.id : id, div);
    }

    console.log(videosMap);

    for (const [i, participantId] of [...participants].entries()) {
        const div = videosMap.get(participantId) || getVideoElement('localVideo').parentElement;
        ...
        if (!!posSz) {
            ...
            div.style.setProperty('left', '0');q
        }
    }
}
...
window.addEventListener('message', (event) => {
    ...
    if (parsed.type === 'set_layout') {
        ...
        currentLayoutIndex = possibleLayouts.findIndex((l) => l.type === parsed.data);
