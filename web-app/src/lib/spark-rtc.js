...
export class SparkRTC {
  ...
  setupSignalingSocket = async (url, myName, roomName, debug) => {
    //if socket is valid no need to create new socket..
    if (await this.checkSocketStatus()) {
      this.updateTheStatus(`Socket is already OPEN no need to Setup Again!`);
      return;
    }
  ...
  newPeerConnectionInstance = (target, theStream, isAudience = false) => {
    ...
    peerConnection.ontrack = async (event) => {
      ...
      if (stream && stream.active) {
        ...
        if (this.remoteStreamCallback && stream.active) {
  ...
  restart = async (closeSocket) => {
    //check socket open status
    if (this.socket) {
      if (this.socket.readyState === WebSocket.CONNECTING) {
        //not opened yet, still in opening state... Go Back & wait
        this.updateTheStatus(
          `Socket is still in CONNECTING State.. Returning..!`
        );
        return;
      } else if (this.socket.readyState === WebSocket.CLOSING) {
        //not opened yet, still in opening state... Go Back & wait
        this.updateTheStatus(`Socket is still in CLOSING State.. Wait for it!`);
        await this.wait(5000);
      }
    }

    this.updateTheStatus("Socket Ready state is: ", this.socket.readyState);
