...
export class SparkRTC {
  ...
  getSupportedCodecs() {
    if (this.supportsSetCodecPreferences) {
      ...
      this.codecs = capabilities.codecs.filter((codec) => codec.mimeType === 'video/H264')
  ...
  handleMessage = async (event) => {
    ...
    switch (msg.type) {
      ...
      case 'alt-video-answer':
        ...
        if (this.iceCandidates && this.iceCandidates.length > 0 && audiencePeerConnection && audiencePeerConnection.remoteDescription) {
          try {
      ...
      case 'alt-new-ice-candidate':
        ...
        if (audiencePeerConnection && audiencePeerConnection.remoteDescription && this.iceCandidates && this.iceCandidates.length > 0) {
          try {
            while (this.iceCandidates.length) {
              await audiencePeerConnection.addIceCandidate(this.iceCandidates.pop())
            }
  ...
  removeFromAcceptedRequests = (data) => {
    if (this.acceptedRequests.includes(data) && this.role === this.Roles.BROADCAST) {
      console.log('removing from acceptedRequests: ', this.acceptedRequests.length)
      var index = this.acceptedRequests.indexOf(data)
      if (index > -1) {
        this.acceptedRequests.splice(index, 1)
      }

      console.log('removed from acceptedRequests: ', this.acceptedRequests.length)
    }
  }

  removeFromSentRequest = (data) => {
    if (this.sentRequests.includes(data) && this.role === this.Roles.BROADCAST) {
  ...
  setResolution = async (stream) => {
    if (stream) {
      ...
      await Promise.all(videoTracks.map((track) => track.applyConstraints(this.resolutionConstraints)))
    }
  }
  ...
  onRaiseHandRejected = () => {
    ...
    if (this.localStream) {
      ...
      this.remoteStreams = this.remoteStreams.filter((stream) => stream.id !== this.localStream.id)

      if (pc && pc.getSenders) {
        this.localStream.getTracks().forEach((track) => {
          const sender = pc.getSenders().find((sender) => sender?.track?.id === track.id)
          if (sender) {
  ...
  restartEverything(peerConnection, target, isAudience) {
    ...
    const allStreams = peerConnection.getRemoteStreams()
    this.updateTheStatus(`All Remote streams of PC`, { allStreams })
    this.updateTheStatus(`All Remotestreams from List before`, this.remoteStreams)
    ...
    if (this.parentStreamId && allStreams.some((s) => s.id === this.parentStreamId)) {
  ...
  newPeerConnectionInstance = (target, theStream, isAudience = false) => {
    ...
    peerConnection.onconnectionstatechange = (event) => {
      this.updateTheStatus(`Connection state: ${peerConnection.connectionState}, pc: `, peerConnection)
            ...
            if (this.firefoxAgent || this.safariAgent) {
              ...
              this.remoteStreams.forEach((stream) => {
                if (stream.id === theEventStream.id) {
                  ...
                  let newArray = this.remoteStreams.filter((str) => str !== stream)
              ...
              if (this.remoteStreams.length > 0) {
                for (var i = 0; i < this.remoteStreams.length; i++) {
                  this.updateTheStatus(`RemoteStreamsList-1`, this.remoteStreams[i])
                }
              }
  ...
  checkBrowser() {
    ...
    this.IExplorerAgent = userAgentString.includes('MSIE') || userAgentString.includes('rv:')
    this.firefoxAgent = userAgentString.includes('Firefox')
    this.safariAgent = userAgentString.includes('Safari') && !userAgentString.includes('Chrome')
    ...
    if (this.remoteStreams.length > 0) {
      ...
      this.remoteStreams.forEach(async (astream) => {
        ...
        astream.getTracks().forEach(async (track) => {
          try {
            let sender = this.myPeerConnectionArray[audienceName].addTrack(track, astream)
  ...
  updatePeerConnectionParams = async (sender) => {
    if (sender) {
      ...
      await this.setVideoSettings(sender, videoSettings.fps, videoSettings.bitrate, resolutionScale)
    }
  }
  /**
  ...
  addCodecPrefrences = async (peerConnection, stream) => {
    if (this.codecs && this.codecs.length > 0) {
      const transceiver = peerConnection.getTransceivers().find((t) => t.sender && t.sender.track === stream.getVideoTracks()[0])
      if (transceiver) {
  ...
  onMediaDevicesChange = async (devices) => {
    if (devices) {
      ...
      if (audioOutputDevices.length > 0) {
        const deviceWithDefaultLabel = audioOutputDevices.find((device) => device.deviceId.toLowerCase().includes('default'))
      ...
      if (audioInputDevices.length > 0) {
        const deviceWithDefaultLabel = audioInputDevices.find((device) => device.deviceId.toLowerCase().includes('default'))
        if (deviceWithDefaultLabel) {
      ...
      if (videoInputDevices.length > 0) {
        const deviceWithDefaultLabel = videoInputDevices.find((device) => device.label.toLowerCase().includes('integrated'))
        if (deviceWithDefaultLabel) {
  ...
  getStatsForPC = (peerConnection, userid) => {
    ...
    if (this.debug) {
      ...
      const checkStats = () => {
        if (peerConnection && (peerConnection.connectionState === 'closed' || peerConnection.connectionState === 'disconnected')) {
  ...
  checkNetworkSpeed = () => {
    if (this.debug) {
      ...
      this.networkSpeedInterval = setTimeout(this.checkNetworkSpeed, this.statsIntervalTime)
    }
  }
