...
export class SparkRTC {
  ...
  handleMessage = async (event) => {
    ...
    switch (msg.type) {
      ...
      case "reconnect":
        this.updateTheStatus(`reconnect`, msg);
        this.startProcedure(true);
        break;
    ...
    if (stream) {
      ...
      this.remoteStreams.forEach((STR) => {
        if (STR.id === stream.id) {
          ...
          let newArray = this.remoteStreams.filter((str) => str !== STR);
    ...
    try {
      ...
      for (const userId in this.myPeerConnectionArray) {
        ...
        this.shareStream.getTracks().forEach(async (track) => {
          let sender = apeerConnection.addTrack(track, this.shareStream);
  ...
  restartEverything(peerConnection, target, isAudience) {
    ...
    try {
      if (this.remoteStreamDCCallback)
        this.remoteStreamDCCallback(peerConnection.getRemoteStreams()[0]);
    } catch {}
  ...
  newPeerConnectionInstance = (target, theStream, isAudience = false) => {
    ...
    peerConnection.ondatachannel = (event) => {
      ...
      receive.onmessage = (e) => {
        ...
        if (!displyedStream) {
          this.updateTheStatus(`insideDisplay`, this.broadcastersMessage);
          ...
          users.forEach((user) => {
            ...
            if (user) {
              ...
              if (user.video === undefined) {
                this.getLatestUserList();
                this.updateTheStatus(`going to fetch latestuserlist`);
                return;
              }
              ...
              if (user.video !== null && user.video !== undefined) {
                if (user.video.id === stream.id) {
                  this.updateTheStatus("video stream id matched");
                  const data = JSON.parse(user.name);
                  userName = data.name;
                } else {
                  this.updateTheStatus("video stream id not matched");
                }
              } else {
        ...
        .then(function (stats) {
          stats.forEach(function (report) {
            ...
            if (report.type === "remote-inbound-rtp") {
              ...
              self.updateTheStatus(`userid: ${userid} Media kind`, report.kind);
              // Packet Loss
              self.updateTheStatus(
              ...
              // Jitter
              self.updateTheStatus(`userid: ${userid} Jitter`, report.jitter);
  ...
  checkNetworkSpeed = () => {
    ...
    if (connection && navigator.onLine) {
      ...
      if (connection.downlink) {
        this.updateTheStatus(`Download Speed: ${connection.downlink} Mbps`);
      }
    } else {
