...
const pathpattern string = "/hapi/v1/rtc-rooms/{" + roomIDname + "}"
...
func serveHTTP(responsewriter http.ResponseWriter, request *http.Request) {
	...
	var room *rtc_rooms.RoomModel
	{
		var err error

		room, err = rtcroomsrv.Repository.GetRoom(roomID)
		if nil != err {
			switch {
			case erorr.Is(err, rtc_rooms.ErrRoomNotFound):
	...
	var bytes []byte
	{
		...
		var id string = rtc_rooms.RoomURL(roomID, path, request.Host)
