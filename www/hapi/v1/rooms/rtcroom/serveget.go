...
const path string = "/hapi/v1/rtc-rooms"
...
func serveGET(responsewriter http.ResponseWriter, request *http.Request) {
	...
	var items []Item
	{
		err := rtcroomsrv.Repository.ForEachRoom(func(roomModel *rtc_rooms.RoomModel) error {
			var id string = rtc_rooms.RoomURL(roomModel.ID, path, request.Host)
	...
	var numRooms int = rtcroomsrv.Repository.NumRooms()
