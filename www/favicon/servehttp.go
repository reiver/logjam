...
func serveHTTP(responsewriter http.ResponseWriter, request *http.Request) {
	...
	var digest string
	{
		digestBytes := sha256.Sum256(favicon)
	...
	_, err := responsewriter.Write(favicon)
	if nil != err {
		log.Errorf("problem writing favicon.ico content to client: %s", err)
	}
}
